...
public class HabitDao {
    ...
    public boolean updateHabit(Habit habit) {
        ...
        Connection connection = null;
        PreparedStatement pst = null;
        String sql = "update Habit set name = ?, icon = ?, color = ?, category = ?, weekday = ?, flag_auto = ? where id = ? and user_id = ?";
        try {
            connection = DatabaseUtil.getConnection();
            connection.setAutoCommit(false);
            pst = connection.prepareStatement(sql);
            ...
            pst.setInt(3, habit.getColor());
            pst.setInt(4, habit.getCategory());
            pst.setInt(5, habit.getWeekday());
            pst.setBoolean(6, habit.isFlag_auto());
            pst.setInt(7, habit.getId());
            pst.setString(8, habit.getUserId());
            if(pst.executeUpdate() != 1) {
                flag = false;
            }
            connection.commit();
    ...
    public boolean clockIn(int id, boolean op, String userId) {
    ...
    public static void main(String[] args) {
        ...
        //findTodayHabit
        List<Habit> list = a.findTodayHabit("user2");
...
//        System.out.println(DateUtil.getDayOfWeek());
    ...
    public List<Habit> findAllHabitByUserId(String userId, int category) {
        //查询所有习惯时使用，-1代表所有习惯
        Connection connection = null;
        PreparedStatement pst = null;
        ResultSet rs = null;
    ...
    public List<Habit> findTodayHabit(String userId) {
        Connection connection = null;
        PreparedStatement pst = null;
        ResultSet rs = null;
        List<Habit> list = new ArrayList<>();
